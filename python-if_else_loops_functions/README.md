...
i % 3 == 0 and i % 5 == 0: Check if i is divisible by both 3 and 5 (multiples of 15). If true, print "FizzBuzz".
i % 3 == 0: Check if i is divisible by 3. If true, print "Fizz".
i % 5 == 0: Check if i is divisible by 5. If true, print "Buzz".
else: If none of the above conditions are true, print the number.
end=" ": Ensures that the numbers and words are printed on the same line, separated by a space.
Testing
You can test the function with the following 12-main.py script:

Expected Output
Running ./12-main.py | cat -e should produce the following output:
